...
myForm.addEventListener('submit',(e)=>{
    ...
    if(myForm.checkValidity&&passwordMatches){
        ...
        document.querySelector('#final-result').textContent='Successfully submitted!'
        myForm.reset();
    }
});
    ...
    const checkName=()=>{
        if(!myName.checkValidity()){
            if(myName.validity.tooShort){
                document.querySelector('#name-text').textContent="Name must consist of more than three characters!"
            }
            else if(myName.validity.valueMissing){
                document.querySelector('#name-text').textContent="Please fill the name field!";
            }
        }
        else{
    ...
    const checkEmail=()=>{
        if(!myEmail.checkValidity()){
            if(myEmail.validity.typeMismatch){
                document.querySelector('#email-text').textContent="Invalid email format!";
            }
            else if(myEmail.validity.valueMissing){
                document.querySelector('#email-text').textContent="Please fill the email field!";
            }
        }
        else{
    ...
    const checkZip=()=>{
        if(!myZip.checkValidity()){
            if(myZip.validity.patternMismatch){
                document.querySelector('#zip-text').textContent="Invalid zip code format";
            }
            else if(myZip.validity.valueMissing){
                document.querySelector('#zip-text').textContent="Please fill the zip code field!";
            }
        }
        else{
    ...
    const checkPassword=()=>{
        if(!myPassword.checkValidity()){
            if(myPassword.validity.tooShort){
                document.querySelector('#password-text').textContent="Enter a stronger password with at least 5 characters/digits";
            }
            else if(myPassword.validity.valueMissing){
                document.querySelector('#password-text').textContent="Please enter a new password";
            }
        }
        else{
    ...
    const checkConfirmPassword=()=>{
        ...
        if(myPassword.checkValidity()){
            if(myConfirm.validity.valueMissing){
                document.querySelector('#confirm-text').textContent="Please confirm your password!";
            }
            else if(myPassword.value!==myConfirm.value){
                document.querySelector('#confirm-text').textContent="Password does't match!"
            }
            else if(myPassword.value===myConfirm.value){
